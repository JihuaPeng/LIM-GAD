...
class HungarianMatcher(nn.Module):
    ...
    def _get_cost_code(self, sim, targets_membership):
        sim_f32 = sim.float() if sim.dtype != torch.float32 else sim
        targets_f32 = targets_membership.float() if targets_membership.dtype != torch.float32 else targets_membership

        cost_code = torch.cdist(sim_f32, targets_f32, p=2)
        
        if sim.dtype == torch.float16:
            cost_code = cost_code.half()
        return cost_code
