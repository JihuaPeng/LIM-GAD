...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6, num_actor=14,
        ...
        decoder_layer_llava = TransformerDecoderLayer_llava(d_model, nhead, num_actor, dim_feedforward, dropout, activation)

        self.decoder = TransformerDecoder(decoder_layer_llava, decoder_layer, num_encoder_layers, decoder_norm,
    ...
    def forward(self, llava_embedding_group, llava_embedding_act, src, actor_mask, group_dummy_mask, group_embed, pos_embed, actor_embed):
        ...
        hs, actor_att, feature_att = self.decoder(llava_embedding_group, llava_embedding_act, tgt, src, attn_mask=actor_mask,
...
class TransformerDecoder(nn.Module):
    def __init__(self, decoder_layer_llava, decoder_layer, num_layers, norm=None, return_intermediate=False):
        super().__init__()
        self.num_layers = num_layers
        self.norm = norm
        self.return_intermediate = return_intermediate

        self.layer_llava = decoder_layer_llava

        self.layers_front = _get_clones(decoder_layer, 3)

        self.layers_back = _get_clones(decoder_layer, 3)



    def forward(self, llava_embedding_group, llava_embedding_act, tgt, memory,
        ...
        output, actor_att, feature_att = self.layer_llava(llava_embedding_group, llava_embedding_act, output, memory, tgt_mask=tgt_mask,
                                                   memory_mask=memory_mask,
                                                   tgt_key_padding_mask=tgt_key_padding_mask,
                                                   memory_key_padding_mask=memory_key_padding_mask,
                                                   attn_mask=attn_mask,
...
class TransformerDecoderLayer_llava(nn.Module):
    ...
    def __init__(self, d_model, nhead, num_actor, dim_feedforward=2048, dropout=0.1, activation="relu"):
        ...
        self.attn_llava_group = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        self.attn_llava_actors = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
    ...
    def forward_post(self, llava_embedding_group, llava_embedding_act, tgt, memory,
                     tgt_mask: Optional[Tensor] = None,
                     memory_mask: Optional[Tensor] = None,
                     tgt_key_padding_mask: Optional[Tensor] = None,
                     memory_key_padding_mask: Optional[Tensor] = None,
        ...
        # actor-llava attention
        tgt_actor_llava, tgt_actor_weights = self.attn_llava_actors(query=tgt_actor, key=llava_embedding_act, value=llava_embedding_act)

        # group-llava attention
        tgt_group_llava, _ = self.attn_llava_group(query=tgt_group, key=llava_embedding_group, value=llava_embedding_group)

        tgt2 = torch.cat([tgt_actor_llava, tgt_group_llava], dim=0)
        ...
        return tgt, tgt2_group, tgt_actor_llava
    ...
    def forward(self, llava_embedding_group, llava_embedding_act, tgt, memory,
                ...
                pos: Optional[Tensor] = None,
                query_pos: Optional[Tensor] = None):
        return self.forward_post(llava_embedding_group, llava_embedding_act, tgt, memory, tgt_mask, memory_mask, tgt_key_padding_mask,
                                 memory_key_padding_mask, attn_mask, pos, query_pos)
