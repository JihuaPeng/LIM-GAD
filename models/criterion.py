...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, num_boxes, log=True):
        ...
        if src_logits_log.dim() == 3:
            src_logits_log = src_logits_log.squeeze(0)

        if tgt_classes_log.dim() == 3:
            tgt_classes_log = tgt_classes_log.squeeze(0)
    ...
    def loss_group_code(self, outputs, targets, group_indices, log=True):
        ...
        for batch_idx in range(sim.shape[0]):
            ...
            sim_batch = sim_batch.float()
            # with torch.cuda.amp.autocast():
            loss_membership += F.binary_cross_entropy(sim_batch, target_members_batch)
                # loss_membership += F.binary_cross_entropy_with_logits(sim_batch, target_members_batch)
    ...
    def forward(self, outputs, targets, log=True):
        ...
        for loss in self.losses:
            losses.update(self.get_loss(loss, outputs, targets, num_boxes))     #loss: labels, outputs: 
